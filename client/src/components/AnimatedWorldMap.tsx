...
const AnimatedWorldMap: React.FC<AnimatedWorldMapProps> = ({ className = "" }) => {
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // Generate random delay for each cell
    ...
    if (containerRef.current) {
      ...
      // Create 35 rows × 55 columns = 1925 cells
      const totalCells = 35 * 55;
      
      // Continent cell indices (converted from the original SCSS)
      ...
      for (let i = 0; i < totalCells; i++) {
        ...
        cell.className = 'world-map-cell';
        
        // Determine continent and set animation with warm colors
        if (northAmerica.includes(i)) {
          cell.style.setProperty('--continent-color', '#f59e0b'); // Warm amber
          cell.style.setProperty('--animation-duration', '8s'); // Slower: was 4s
          cell.style.setProperty('--animation-delay', `${generateRandomDelay(8000)}ms`); // Slower: was 4000ms
        } else if (southAmerica.includes(i)) {
          cell.style.setProperty('--continent-color', '#f97316'); // Warm orange
          cell.style.setProperty('--animation-duration', '6s'); // Slower: was 3s
          cell.style.setProperty('--animation-delay', `${generateRandomDelay(6000)}ms`); // Slower: was 3000ms
        } else if (europe.includes(i)) {
          cell.style.setProperty('--continent-color', '#ef4444'); // Warm red
          cell.style.setProperty('--animation-duration', '6s'); // Slower: was 3s
          cell.style.setProperty('--animation-delay', `${generateRandomDelay(6000)}ms`); // Slower: was 3000ms
        } else if (africa.includes(i)) {
          cell.style.setProperty('--continent-color', '#eab308'); // Golden yellow
          cell.style.setProperty('--animation-duration', '8s'); // Slower: was 4s
          cell.style.setProperty('--animation-delay', `${generateRandomDelay(8000)}ms`); // Slower: was 4000ms
        } else if (asia.includes(i)) {
          cell.style.setProperty('--continent-color', '#dc2626'); // Deep red
          cell.style.setProperty('--animation-duration', '10s'); // Slower: was 5s
          cell.style.setProperty('--animation-delay', `${generateRandomDelay(10000)}ms`); // Slower: was 5000ms
        } else if (australia.includes(i)) {
          cell.style.setProperty('--continent-color', '#ea580c'); // Burnt orange
          cell.style.setProperty('--animation-duration', '4s'); // Slower: was 2s
          cell.style.setProperty('--animation-delay', `${generateRandomDelay(4000)}ms`); // Slower: was 2000ms
        } else {
          // Empty cells
  ...
  return (
    <div className={`animated-world-map ${className}`}>
      <div 
        ref={containerRef}
