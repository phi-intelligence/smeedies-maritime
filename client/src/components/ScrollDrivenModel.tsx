...
const ScrollDrivenModel: React.FC<ScrollDrivenModelProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Configure loader to handle texture loading issues
    // loader.setDRACOLoader(null); // Disable DRACO if causing issues
    
    // Set up texture loading with proper error handling
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        // Create a black material for continent meshes
        const continentMaterial = new THREE.MeshStandardMaterial({
          color: 0x000000, // Black color
          metalness: 0,
          roughness: 1.0,
          emissive: 0x111111, // Very dark glow
          emissiveIntensity: 0.05,
          side: THREE.DoubleSide, // Render both sides to avoid hollow spaces
          transparent: false // Ensure solid appearance
        });

        // Analyze all meshes in the model and apply proper categorization
        let meshCount = 0;
        let materialCount = 0;
        let shipMeshCount = 0;
        let containerMeshCount = 0;
        let greenMeshCount = 0;
        const materials = new Set();
        
        // Ship materials (based on our analysis) - THESE STAY UNCHANGED
        const shipMaterials = ['Ship.001', 'Ship.002', 'Ship.003', 'Ship.004'];
        
        // Container detection patterns - THESE STAY UNCHANGED
        // Based on our analysis, containers use colored materials (red, blue, yellow)
        const containerMaterials = [
          // Red container materials
          'Material.001', 'Material.003', 'Material.004', 'Material.006', 'Material.007',
          'Material.009', 'Material.011', 'Material.013', 'Material.016', 'Material.019',
          'Material.024', 'Material.026', 'Material.027', 'Material.028', 'Material.030',
          'Material.032', 'Material.033', 'Material.034', 'Material.035', 'Material.039',
          
          // Blue container materials  
          'Material.042', 'Material.045', 'Material.048', 'Material.051', 'Material.052',
          'Material.053', 'Material.054', 'Material.055', 'Material.057', 'Material.058',
          'Material.060', 'Material.061', 'Material.065', 'Material.067', 'Material.068',
          'Material.071',
          
          // Yellow container materials
          'Material.072', 'Material.073', 'Material.074', 'Material.075', 'Material.076'
        ];
        ...
        model.traverse((child) => {
          if (child instanceof THREE.Mesh) {
            ...
            // Determine mesh type - SIMPLE LOGIC: Only ships and containers stay unchanged
            const isShipMesh = shipMaterials.includes(materialName);
            const isContainerMesh = containerMaterials.includes(materialName) || containerPatterns.some(pattern => pattern.test(meshName));
            ...
            if (isShipMesh) {
              ...
              // EVERYTHING ELSE becomes green (continents, black regions, etc.)
              meshType = 'green';
              greenMeshCount++;
            }
            ...
            // Apply styling based on mesh type - SIMPLE APPROACH
            if (isShipMesh) {
              console.log(`  → Ship mesh detected: ${meshName} (${materialName}) - keeping original material`);
              
              // Keep ship materials as they are - ships remain unchanged
              // Ships use Ship.001, Ship.002, Ship.003, Ship.004 materials
            } else if (isContainerMesh) {
              console.log(`  → Container mesh detected: ${meshName} - keeping colorful materials`);
              
              // Containers keep their colorful materials (red, blue, yellow)
              // No changes needed - containers remain colorful
            } else {
              console.log(`  → Making mesh black: ${meshName} (${materialName}) - applying black color`);
              
              // EVERYTHING ELSE becomes black (all continents, black regions, etc.)
              child.material = continentMaterial;
        ...
        console.log('Ship meshes (unchanged):', shipMeshCount);
        console.log('Container meshes (unchanged):', containerMeshCount);
        console.log('Black meshes (everything else):', greenMeshCount);
        ...
        model.traverse((child) => {
          if (child instanceof THREE.Mesh) {
            ...
            // Fix material issues while preserving original appearance
            if (child.material) {
              if (Array.isArray(child.material)) {
      ...
      <div className="absolute top-4 left-4 z-50 bg-black/80 text-white p-3 rounded text-xs">
        ...
        {isModelLoaded && (
          <div className="mt-2 text-gray-300 text-xs">
            <div>🚢 4 Cargo Ships (Unchanged)</div>
            <div>📦 41 Colored Containers (Unchanged)</div>
            <div>⚫ Everything Else → BLACK</div>
            <div className="text-blue-300">
              <div>Ship.001, Ship.002, Ship.003, Ship.004</div>
              <div>Red, Blue, Yellow containers</div>
            </div>
            <div className="text-gray-400 mt-1">
              <div>⚫ ALL CONTINENTS & REGIONS → BLACK</div>
              <div>⚫ Simple Logic: Only Ships & Containers Stay Same</div>
              <div>⚫ Everything Else = Black Color (0x000000)</div>
            </div>
          </div>
        )}
      </div>
...
export default ScrollDrivenModel;
